...
public class StatsUserNewInstallUserCollector implements IOutputCollector {
    ...
    public void collect(Configuration conf, BaseDimension key, BaseStatsValueWritable value, IDimensionConverter converter, PreparedStatement pstmt) throws IOException, SQLException {
        ...
        int i = 0;
        pstmt.setInt(++i, converter.getDimensionIdByValue(userDimension.getCommonDimension().getPlatformDimension()));
        pstmt.setInt(++i, converter.getDimensionIdByValue(userDimension.getCommonDimension().getDateDimension()));
